...
Model::Model(const char* path, double size, vec3 position) {
	this->size = size;
...
void Model::setPositionX(float newX) {
	position.x = newX;
}

void Model::setPositionY(float newY) {
	position.y = newY;
}

void Model::setPositionZ(float newZ) {
	position.z = newZ;
}

void Model::setSize(double newSize) {
	size = newSize;
}
